...
reset:
	...
	ser tmp			; Set PORTD (LSD) as output
	out DDRD, tmp		;
	out DDRB, tmp

	; ser tmp out PORTB, tmp	
	; rcall lcd_init		; Initialize LCD
	; rcall lcd_clear_display
	; clr tmp out PORTB, tmp
...
start:
	...
	ldi r24, LOW(100)	; Set delay time to 100ms
	ldi r25, HIGH(100)	;
	rcall wait_msec		; Loop with delay
	rjmp start
...
div_cont:
	...
	rjmp div_loop
div_done:
	mov r0, res0 mov res0, argl
	mov r1, res1 mov res1, argh
...
/* wait_msec:
	...
	ret ; 4 cycles */
wait_msec: ; delay of 1000*F1+6 cycles (almost equal to 1000*F1 cycles){{{
