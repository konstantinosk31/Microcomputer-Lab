 ...
 * main.c
 *
 * Created: 11/22/2024 10:09:08 AM
 *  Author: User
 */ 
...
void char_to_lcd(){
	...
	if(!c || c == last_char) return;
	last_char = c;
	lcd_clear_display();
	if('0' <= c && c <= '9') lcd_digit(c-'0');
	else if('A' <= c && c <= 'D') lcd_data(c);
	else if(c == '*') lcd_data(0b00101010);
	else if(c == '#') lcd_data(0b00100011);
}
